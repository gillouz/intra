...
$info_query="select info_".$lang." as info from offre_verre_info where '$param[designation]' like concat('%',name,'%') and `_status`<9;";
...
try 
{
  ...
  $conn = new PDO($databases["binew"]["dsn"], $databases["binew"]["user"], $databases["binew"]["password"]);
  $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  $conn->exec("SET CHARACTER SET utf8");
  $conn->exec("use ".$databases["binew"]["dbname"]);*/
  //$conn->exec("use dwh_dev");
...
//$html.="<div class='row'><h4>".nano\lbl("info")."</h4></div>";
...
$html.="<p class=''><a class='pull-right' href='page_cat_lens_config.php'>".nano\lbl("help_us_cat_lens_config")."</a></p>";

$html.="</div></div>";

// ----------------------- price

$html.="<div class='row' ><h4>".nano\lbl("price")."</h4></div>";
...
foreach($option as $res)
{
  ...
  if($res["color"]!=$color) $html.="<div style='clear:both'><hr>".nano\lbl("cle_option_color_".$res[color])."</div>";
  $color=$res["color"];

  $html.="<div  class='tile-small  tile-small-center tile-gray-$tcolor col-xs-6 col-md-4 col-lg-3' ><div class='tile-text'>
  $res[coating_designation] <strong>$res[coating_pv]</strong> CHF</div>
  </div>
  ";
